...
$axure.internal(function($ax) {
    ...
    var _unregister = function(label) {
        ...
        if(regionIndex != -1) {
            var end = regionList.splice(regionIndex + 1);
            regionList.splice(regionIndex, regionList.length - regionIndex);
    ...
    var _genRect = function(info) {
        // 3 senarios: A panel that is fixed, an widget in a panel that is fixed, and others
        var obj = $obj(info.elementId);

        // You are a panel and fixed
        if(obj.type === 'dynamicPanel' && obj.fixedHorizontal) return _getPanelRect(info, obj);

        var parentPanelRect = _getParentPanelRect(info);
        if(parentPanelRect) return parentPanelRect;
    ...
    var _getPanelRect = function(widgetInfo, obj) {
        var thisWidth = widgetInfo.width;
        var thisHeight = widgetInfo.height;

        return {
            X: function() {
                var window = $ax.getWindowInfo();
                var scrollX = window.scrollx;
                var windowWidth = window.width;
                
                var horzAlign = obj.fixedHorizontal;
                var horzMargin = obj.fixedMarginHorizontal;
                if(horzAlign == 'right') return scrollX + windowWidth - horzMargin;
                if(horzAlign == 'center') return scrollX + (windowWidth - thisWidth) / 2 + horzMargin;
                // Otherwise left
                return scrollX + horzMargin;
            },
            Y: function() {
                var window = $ax.getWindowInfo();
                ...
                var vertAlign = obj.fixedVertical;
                var vertMargin = obj.fixedMarginVertical;
                if(vertAlign == 'bottom') return scrollY + windowHeight - vertMargin;
                if(vertAlign == 'middle') return scrollY + (windowHeight - thisHeight) / 2 + vertMargin;
                // Otherwise top
                return scrollY + vertMargin;
            },
            Width: function() { return thisWidth; },
            Height: function() { return thisHeight; },
    ...
    var _getParentPanelRect = function(widgetInfo) {
        var parents = $ax('#' + widgetInfo.elementId).getParents(true)[0];
        var fixedParentPanel = undefined;
        for(var i = 0; i < parents.length; i++) {
            var parentId = parents[i].split('_')[0];
            var parentObj = $obj(parentId);
            if(parentObj.type != 'dynamicPanel' || !parentObj.fixedHorizontal) continue;
            fixedParentPanel = parentId;
        }
        if(!fixedParentPanel) return undefined;

        // Get rect for the parent panel
        var panelRect = _getPanelRect($ax.getWidgetInfo(fixedParentPanel), $obj(fixedParentPanel));

        // Calculate your offset.
        var offset = $ax.move.DivToDiagramCoordinates(widgetInfo.elementId, widgetInfo.x, widgetInfo.y, false, function(id, type) {
            if(type != 'dynamicPanel') return false;
            return Boolean($obj(id).fixedHorizontal);
        });

        return {
            X: function() { return panelRect.X() + offset.x; },
            Y: function() { return panelRect.Y() + offset.y; },
            Width: function() { return widgetInfo.width; },
            Height: function() { return widgetInfo.height; },
            Left: function() { return this.X(); },
            Right: function() { return this.X() + this.Width(); },
            Top: function() { return this.Y(); },
            Bottom: function() { return this.Y() + this.Height(); }
