...
$axure.internal(function($ax) {
    ...
    $ax.annotation.ToggleWorkflow = function(event, id, width, height) {
        ...
        //var target = window.document.getElementById(Workflow(id));
        var bufferH = 10;
        var bufferV = 10;
        var blnLeft = false;
        var blnAbove = false;
        var sourceTop = event.pageY - window.document.body.scrollTop;
        var sourceLeft = event.pageX - window.document.body.scrollLeft;

        if(sourceLeft > width + bufferH + window.document.body.scrollLeft) {
            blnLeft = true;
        }
        if(sourceTop > height + bufferV + window.document.body.scrollTop) {
            blnAbove = true;
        }
    ...
    $(window.document).ready(function() {
        $ax(function(dObj) { return dObj.annotation; }).each(function(dObj, elementId) {
